...
import { TEM } from "@/types";
import { db } from "@/lib/db";

export default function TEMsPage() {
  const navigate = useNavigate();
  const [list, setList] = useState<TEM[]>([]);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState({ codigo: "", descricao: "" });

  const load = () => setList(db.list<TEM>("tems"));
  useEffect(() => { load(); }, []);

  const reset = () => { setEditingId(null); setForm({ codigo: "", descricao: "" }); };

  const edit = (item: TEM) => { setEditingId(item.id); setForm({ codigo: item.codigo, descricao: item.descricao }); };

  const save = () => {
    if (!form.codigo.trim()) return;
    if (editingId) {
      db.update<TEM>("tems", editingId, form);
    } else {
      db.create<TEM>("tems", { ...form, createdAt: new Date().toISOString() });
    }
    reset(); load();
  };

  const remove = (id: string) => {
    if (!confirm("Excluir este código TEM?")) return;
    db.remove("tems", id);
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl sm:text-2xl font-bold text-slate-800">Códigos TEM</h1>
      </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4 sm:p-5 mb-6">
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-end">
          <div className="flex-1">
            ...
            <input type="text" value={form.codigo} onChange={(e) => setForm({ ...form, codigo: e.target.value })} className={inp} placeholder="Ex: TEM 121" />
          </div>
          <div className="flex-1">
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm">
        {list.map((item) => (
          <div key={item.id} className="p-3 sm:p-4 border-b border-slate-100 last:border-0 flex items-center justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              {item.descricao && <span className="text-sm text-slate-500 ml-2 truncate">{item.descricao}</span>}
            </div>
        ...
        {list.length === 0 && <p className="text-center text-slate-500 py-8">Nenhum código TEM cadastrado</p>}
      </div>
    </div>
  );
}
