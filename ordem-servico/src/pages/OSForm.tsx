...
export default function OSForm() {
  ...
  useEffect(() => {
    ...
    // Auto-seleciona o primeiro TEM, primeira Revisão e setor Manutenção se não estiver editando
    if (!id) {
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate("/")} className="p-2 rounded-md text-slate-500 hover:bg-slate-200 transition-colors">
          <ArrowLeft className="h-5 w-5" />
        </button>
        <h1 className="text-2xl font-bold text-slate-800">
          {isEditing ? "Editar OS" : isCorretiva ? "Nova OS" : "Nova Preventiva"}
          {isEditing && id && db.get<OrdemServico>("ordens", id)?.numero && <span className="ml-2 text-base font-normal text-slate-400">#{db.get<OrdemServico>("ordens", id)?.numero}</span>}
        </h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5">
          ...
          <div className="space-y-4">
            {form.atividades.map((osAtv) => {
              ...
              if (!tmpl) return null;
              return (
                <div key={osAtv.templateId} className="border border-slate-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold text-slate-800 text-sm">{tmpl.nome}</h3>
                    <button type="button" onClick={() => removeAtividade(osAtv.templateId)} className="p-1 rounded-md text-slate-400 hover:text-red-600">
                  ...
                  <div className="space-y-2">
                    {osAtv.itens.map((osItem) => {
                      ...
                      return (
                        <div key={osItem.itemId} className="flex items-center justify-between gap-3 py-2 border-b border-slate-100 last:border-0">
                          <span className="text-sm text-slate-700">{nome}</span>
                          <div className="flex items-center gap-2 shrink-0">
                            <button
                              type="button"
                              onClick={() => setItemStatus(osAtv.templateId, osItem.itemId, true)}
                              className={`px-3 py-1.5 rounded text-xs font-semibold transition-colors min-w-[50px] text-center ${
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5">
            ...
            <div>
              ...
              <div className="flex gap-3 mt-1">
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input type="radio" name="seguranca" checked={form.segurancaEquipamento === "CONF"} onChange={() => set("segurancaEquipamento", "CONF")} className="h-4 w-4 text-green-600 focus:ring-green-500" />
                  <span className="text-sm font-medium text-green-700">CONF.</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input type="radio" name="seguranca" checked={form.segurancaEquipamento === "NAO_CONF"} onChange={() => set("segurancaEquipamento", "NAO_CONF")} className="h-4 w-4 text-red-600 focus:ring-red-500" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex gap-3 mt-1">
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input type="radio" name="conclusaoSeguranca" checked={form.conclusaoSeguranca === "CONF"} onChange={() => set("conclusaoSeguranca", "CONF")} className="h-4 w-4 text-green-600 focus:ring-green-500" />
                  <span className="text-sm font-medium text-green-700">CONF.</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input type="radio" name="conclusaoSeguranca" checked={form.conclusaoSeguranca === "NAO_CONF"} onChange={() => set("conclusaoSeguranca", "NAO_CONF")} className="h-4 w-4 text-red-600 focus:ring-red-500" />
                  <span className="text-sm font-medium text-red-700">NÃO CONF.</span>
                </label>
              </div>
            </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5">
          ...
          <div className="flex gap-4 mb-3">
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input type="radio" name="substituicao" checked={form.substituicaoPecas === "sim"} onChange={() => set("substituicaoPecas", "sim")} className="h-4 w-4 text-blue-600 focus:ring-blue-500" />
              <span className="text-sm font-medium">Sim</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input type="radio" name="substituicao" checked={form.substituicaoPecas === "nao"} onChange={() => set("substituicaoPecas", "nao")} className="h-4 w-4 text-blue-600 focus:ring-blue-500" />
              <span className="text-sm font-medium">Não</span>
            </label>
          </div>
          {form.substituicaoPecas === "sim" && (
            <textarea value={form.substituicaoPecasDescricao} onChange={(e) => set("substituicaoPecasDescricao", e.target.value)} className={inputClass + " min-h-[60px]"} placeholder="Descreva as peças substituídas..." />
          )}
        </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5">
          ...
          <div className="flex gap-4 mb-3">
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input type="radio" name="outra" checked={form.outraManutencao === "sim"} onChange={() => set("outraManutencao", "sim")} className="h-4 w-4 text-blue-600 focus:ring-blue-500" />
              <span className="text-sm font-medium">Sim</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input type="radio" name="outra" checked={form.outraManutencao === "nao"} onChange={() => set("outraManutencao", "nao")} className="h-4 w-4 text-blue-600 focus:ring-blue-500" />
              <span className="text-sm font-medium">Não</span>
            </label>
          </div>
          {form.outraManutencao === "sim" && (
            <textarea value={form.outraManutencaoDescricao} onChange={(e) => set("outraManutencaoDescricao", e.target.value)} className={inputClass + " min-h-[60px]"} placeholder="Descreva a outra manutenção..." />
          )}
        </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5">
          ...
          <textarea value={form.observacao} onChange={(e) => set("observacao", e.target.value)} className={inputClass + " min-h-[80px]"} placeholder="Observações adicionais..." />
        </div>
        ...
        <div className="flex justify-end">
          <button type="submit" className="bg-blue-600 text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors flex items-center gap-2">
